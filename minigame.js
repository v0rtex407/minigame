window.onload = () => {
    ...
    window.ontouchmove = e => {
        if (b.style.transition == "opacity 0.75s ease 0s, left 0.05s linear 0s" || b.style.transition == "left 0.05s linear 0s, opacity 0.75s ease 0s" || b.style.transition == "opacity 0.75s ease 0s") b.style.transition = "opacity 0.75s ease 0s"
        else if (b.style.transition == "left 0.05s linear 0s") b.style.transition = ""
        b.style.left = (e.pageX - 25) + "px"
    }
    window.ontouchstart = e => {
        b.style.left = (e.pageX - 25) + "px"
        if (b.style.transition == "opacity 0.75s ease 0s" || b.style.transition == "opacity 0.75s ease 0s, left 0.05s linear 0s" || b.style.transition == "left 0.05s linear 0s, opacity 0.75s ease 0s") {
            b.style.transition = "left 0.05s linear 0s, opacity 0.75s ease 0s"
        ...
        let h = setInterval(() => {
            ...
            else if (b.style.transition == "left 0.05s linear 0s, opacity 0.75s ease 0s") b.style.transition = "opacity 0.75s ease 0s"
            clearInterval(h)
        }, 50)
    }
    ...
    const enemy = () => {
        ...
        const fall = () => {
            ...
            if ((((b.offsetTop <= c.offsetTop + 70) && (b.offsetLeft >= c.offsetLeft - 50 && b.offsetLeft <= c.offsetLeft + 70)) || c.offsetTop >= window.innerHeight - 45) && c.getAttribute("value") != "1" && b.style.transition != "opacity 0.75s ease 0s" && b.style.transition != "left 0.05s linear 0s, opacity 0.75s ease 0s") {
                ...
                else if (b.style.transition == "left 0.05s linear 0s") b.style.transition = "left 0.05s linear 0s, opacity 0.75s ease 0s"
                b.style.opacity = "0.5"
                const glow = () => {
                    b.style.opacity = "1"
